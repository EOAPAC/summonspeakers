...
async function writeManifest(): Promise<number> {
  ...
  // Format with prettier so the generated manifest keeps lint green.
  await writeFile(
    MANIFEST,
    await format(
 ...
 */
export const portraitSlugs: readonly string[] = ${JSON.stringify(
        entries.map(([slug]) => slug),
        null,
        2,
      )};
`,
      { ...(await resolveConfig(MANIFEST)), filepath: MANIFEST },
    ),
  );

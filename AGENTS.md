...
| Speaker portraits      | Wiring is in place (`Portrait.tsx` reads the generated manifest), but no images exist, so all 12 profiles still render `hatch`. Run `bun run build:portraits` where `RUNWARE_API_KEY` and egress to api.runware.ai both exist. Text prompts only — never condition on a photo of a real person. | `scripts/build-speaker-portraits.ts` does the generating. It needs the key and egress **only where it runs** — output is committed, so the deployed site never calls Runware and the key need not be in Vercel or Supabase. Nothing calls an image API per request, which on SSR would be a per-visitor cost. |
